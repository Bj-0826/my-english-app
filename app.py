...
def fmt_baek(val):
    """백만 단위 포맷 (예: 291백만)"""
    baek = val / 1e6
    return f"{int(round(baek))}백만"

def fmt_krw_axis_pension(val, _):
    """연금자산 Y축 포맷: 억 단위"""
    return fmt_eok(val)

def fmt_krw_axis_personal(val, _):
    """개인자산 Y축 포맷: 억 단위"""
    eok = val / 1e8
    if eok >= 1:
        if eok == int(eok):
        ...
        baek = val / 1e6
        if baek == int(baek):
            return f"{int(baek)}천만"
        else:
            return f"{baek:.0f}백만"

# ==========================================
# [공통 유틸] 주차 라벨 변환 함수 (개인자산)
# ==========================================

def week_key_to_label(k):
    """Y2026W19 → 26년 4월 3주 형태 변환"""
    try:
        k = str(k)
        y = int(k[1:5])
        w = int(k.split('W')[1])
...
def label_to_week_key(year, month, week_of_month):
    """(2026, 4, 3) → Y2026W19 형태 변환"""
    # 해당 년도 1월 1일부터 주차 계산
    # 월의 첫 번째 날 찾기
    first_day_of_month = datetime.date(year, month, 1)
    # week_of_month 번째 주의 첫 번째 날 (실제 일자)
    # 1주 = 1~7일, 2주 = 8~14일, 3주 = 15~21일, 4주 = 22~28일, 5주 = 29~31일
        ...
        with c2:
            ...
            with st.form("in_ta_tower"):
                ...
                if st.form_submit_button("통합자산 데이터 저장"):
                    ...
                    conn.update(worksheet="TotalAssets", data=pd.concat([df_ta.drop(columns=['date_dt', 'display_date'], errors='ignore'), new_ta], ignore_index=True))
                    st.rerun()
...
# --- [기존 모드: 일반 모드] ---
    ...
    # =========================================================
    # [1. 연금자산] - Y축 억원, 그래프 상단 총금액, hover 백만원 단위
    # =========================================================
    if menu == "💰 연금자산":
        st.header("💰 연금자산 관리")
        ...
        with t1:
            if not df_p.empty and 'date' in df_p.columns:
                ...
                if dates_sorted_dt:
                    ...
                    # ── 스택 막대 그래프 구성 ──
                    fig = go.Figure()
                    accounts = sorted(df_r['account'].unique())
    ...
    # =========================================================
        ...
        with tab1:
            ...
            with c2:
                ...
                tick_step_eok = 1e8  # 1억 단위 tick
                if max_asset > 20e8:
                    tick_step_eok = 5e8  # 5억 단위
                elif max_asset > 10e8:
                    tick_step_eok = 2e8  # 2억 단위
                ...
                fig_sim.update_layout(
                    ...
                    yaxis=dict(
                        tickvals=tick_vals_sim,
                        ticktext=tick_texts_sim,
                    ),
                    height=450,
    ...
    # =========================================================
        ...
        with t4:
            if not df_cf.empty:
                ...
                if not m_data.empty:
                    # ── 최신순 정렬: date 내림차순, 같은 날짜는 원본 index 내림차순 ──
                    m_data = m_data.sort_values('date', ascending=False)

                    edit_list = m_data.apply(
                        lambda x: f"[{x['date']}] {x['category']} - {x['memo']} ({int(x['amount']):,}원)",
                        axis=1
                    ).tolist()
    ...
    # =========================================================
    # [4. 개인자산] - Y축 억원, 주차 라벨 통일, 그래프 상단 총금액
    # =========================================================
    elif menu == "💵 개인자산":
        st.header("💵 개인자산 관리")
        ...
        with t1:
            if not df_per.empty and 'date' in df_per.columns:
                ...
                if weeks:
                    ...
                    tick_vals_per = list(range(0, int(max_y_per) + int(tick_step_per), int(tick_step_per)))
                    tick_texts_per = []
                    for v in tick_vals_per:
                        eok_v = v / 1e8
                        if eok_v >= 1:
                            if eok_v == int(eok_v):
                                tick_texts_per.append(f"{int(eok_v)}억")
                            else:
                                tick_texts_per.append(f"{eok_v:.1f}억")
                        else:
                            baek_v = v / 1e7
        ...
        with t2:
            ...
            # 연도/월/주차를 선택하면 실제 week_key 자동 계산
