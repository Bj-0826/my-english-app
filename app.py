...
# 1. 앱 기본 설정
st.set_page_config(page_title="Byungjoo Pro v4.0.0", layout="wide")

# 2. 구글 시트 연결 (TTL=0으로 설정하여 캐시 강제 무력화)
...
def handle_save_asset(s_name, date_val, acc, amt_key):
    ...
    try:
        ...
        st.success(f"✅ {acc} 데이터가 시트에 안전하게 저장되었습니다.")
        # [중요] 저장 후 즉시 화면을 새로고침하여 대시보드에 반영
        st.rerun()
...
def handle_save_english():
    ...
    if en and ko:
        ...
        st.toast("✅ 문장이 저장되었습니다.")
        st.rerun()

# --- [UI 구성] ---
with st.sidebar:
    st.title("Byungjoo Pro v4.0")
    menu = st.radio("메뉴 선택", ["💰 연금자산", "💵 개인자산", "🔤 영어공부"])
    st.divider()
    ret_date = datetime.date(2028, 12, 31)
    st.metric("은퇴 D-Day", f"D-{(ret_date - datetime.date.today()).days}")

if menu == "💰 연금자산":
    st.header("💰 연금자산 관리 (Data 탭)")
    df_p = load_data_safe("Data")
    t1, t2 = st.tabs(["📊 대시보드", "📝 데이터 입력"])
    ...
    with t1:
        if not df_p.empty and len(df_p) > 0:
            ...
            c1, c2 = st.columns(2)
            c1.metric(f"{recent_dates[-1]} 총합", f"{int(cur):,}원")
            ...
            st.info("시트에 데이터가 없습니다. 입력 탭에서 데이터를 추가해 주세요.")
    ...
    with t2:
        ...
        p_acc = st.selectbox("자산 항목", ['퇴직연금', 'IRP', 'ISA', '개인연금'])
        st.number_input("금액(원)", step=100000, key="p_amt")
        st.button("시트에 저장하기", on_click=handle_save_asset, args=("Data", f"{py}-{pm}", p_acc, "p_amt"))

elif menu == "💵 개인자산":
    st.header("💵 개인자산 관리 (PersonalData 탭)")
    ...
    with t1:
        if not df_per.empty and len(df_per) > 0:
            ...
            c1, c2 = st.columns(2)
            c1.metric(f"{get_w_label_python(recent_w[-1])} 총합", f"{int(cur):,}원")
    ...
    with t2:
        ...
        p_acc_per = st.selectbox("계좌 선택", ['KB증권', '삼성증권', '카카오', '한투증권', '현금/기타'])
        st.number_input("금액(원)", step=10000, key="per_amt")
        st.button("시트에 저장하기", on_click=handle_save_asset, args=("PersonalData", f"Y{pery}W{perw}", p_acc_per, "per_amt"))

else:
    st.header("🔤 Byungjoo의 영어 공부")
    df_en = load_data_safe("Sheet1")
    t1, t2 = st.tabs(["📖 리스트", "✍️ 문장 추가"])
    with t1:
        if not df_en.empty:
            st.dataframe(df_en[['date', 'english', 'korean', 'memorized']].iloc[::-1], use_container_width=True)
    with t2:
        st.text_input("영어 문장", key="new_en")
        st.text_input("한글 번역", key="new_ko")
