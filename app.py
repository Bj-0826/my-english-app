...
# --- [6. 여행관리] ---
    ...
    if not df_dest.empty:
        ...
        with t_stats:
            col1, col2 = st.columns(2); col1.write("**📅 일자별**"); col1.write(d_exp.groupby('date')['amount'].sum().map(lambda x: f"₩{int(x):,}"))
            col2.write("**💳 결제수단별**"); col2.write(d_exp.groupby('method')['amount'].sum().map(lambda x: f"₩{int(x):,}"))
        with t_edit:
            if not d_exp.empty:
