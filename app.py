...
# 1. 앱 설정
st.set_page_config(page_title="은퇴 준비하기 v5.5.3", layout="wide")

# 2. 구글 시트 연결 (표준 커넥션 방식)
conn = st.connection("gsheets", type=GSheetsConnection)

# --- [데이터 로드 함수: 400 에러 방지를 위해 worksheet 이름만 사용] ---
def load_data_safe(s_name):
    try:
        # Secrets에 등록된 주소를 '부모'로 삼아 탭 이름(worksheet)만 호출합니다.
        # 이렇게 해야 구글 서버에서 HTTP 400 오류를 뱉지 않습니다.
        ...
        # 에러 발생 시 사용자에게 친절히 알림
        st.warning(f"⚠️ '{s_name}' 탭을 읽어오는 중 문제가 발생했습니다. (사유: {e})")
        return pd.DataFrame()
...
# --- [3. 현금흐름] ---
    ...
    with t1:
        ...
        if current_budget == 0:
            ...
            c1, c2 = st.columns(2)
            c1.metric("현재 지출", f"{int(total_spent):,}원"); c2.metric("설정 예산", "0원")
        else:
            percent = (total_spent / current_budget) * 100
            c1, c2, c3 = st.columns(3)
            if percent < 70:
                c1.metric("소비 상태", "✅ 안전", f"{int(current_budget-total_spent):,}원 남음"); st.success(f"예산의 {percent:.1f}% 사용 중. 아주 훌륭합니다!")
            elif percent < 90:
                c1.metric("소비 상태", "🟡 주의", f"{int(current_budget-total_spent):,}원 남음"); st.warning(f"예산의 {percent:.1f}% 사용 중. 지출을 조절하세요.")
            else:
                c1.metric("소비 상태", "🚨 위험", f"{int(current_budget-total_spent):,}원 남음"); st.error("예산 초과 임박! 소비 통제가 필요합니다.")
    with t2:
...
# --- [5. 영어공부] ---
    ...
    with t3:
        if not df_en.empty:
            ...
            sel_en = st.selectbox("수정할 문장", options=edit_list); en_idx = df_en.index[edit_list.index(sel_en)]; en_row = df_en.loc[en_idx]
...
# --- [6. 도서관리 - Byungjoo님의 원본 로직 100% 복구] ---
elif menu == "📚 도서관리":
    st.header("📚 도서 관리 시스템")
...
# --- [7. 여행관리 - 타임라인 CSS 및 디테일 로직 100% 복구] ---
elif menu == "✈️ 여행관리":
